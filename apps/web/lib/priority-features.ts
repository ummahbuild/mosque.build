...
export function getPriorityFeatureBatch(batch:number):PriorityFeature[]{const start=(batch-1)*100;return candidates.slice(start,start+100).map(item=>({id:item.id,title:item.title,description:item.description.trim()||"Description not yet specified. Define the user outcome, scope, failure states, tests and review gate before implementation.",domain:item.domains[0]||"platform",domains:item.domains,status:item.canonical_status,wave:item.recommended_wave,score:item.priority_score,professionalGate:item.requires_professional_or_authority_gate,external:item.uses_paid_or_external_services_likely,ai:item.uses_ai,sourceCount:item.source_refs.length,developmentComplexity:item.development_complexity,provenanceBurden:item.data_provenance_burden}))}
